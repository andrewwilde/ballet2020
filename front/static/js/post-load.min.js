...
function populate_classes(response){
	...
	$.each(response, function(_, classes) {
		var level_classes = [];
		$.each(classes, function(idx, dance_class) {
			...
			if (dance_class['dance_type'] == 'Dance Camp'){
				template_vars['start_day'] = dance_class['start_day'];
				template_vars['end_day'] = dance_class['end_day'];
				let {day, start_time, stop_time, age_min, age_max, class_status, start_day, end_day, description} = template_vars;
				var class_template = `	<div class="ci-metas">
								<div class="ci-meta" style="margin-bottom:0px;"><h6>${start_day} to ${end_day} @ ${start_time} - ${stop_time} ${class_status}</h6><h6>${day}</h6><h6 style="font-size:12px;color:#c687b0">${description}</h6></div>
								<br/><br/>
							</div>`
		...
		var doc_title = "Curriculum";
		if(classes[0]['class_type'] == 'Summer Camp'){
			doc_title = 'Camp Schedule';
		}
		...
		let template_vars = {
			...
			sale_price: classes[0]['sale_price'],
			payment_frequency: classes[0]['payment_frequency'],
			curriculum: curriculum_loc ,
			curriculum_title: doc_title,
			title: classes[0]['dance_category'],
			class_type: classes[0]['class_type'],
		}

		let {image, secondary, ages, times, age_min, age_max, price, sale_price, payment_frequency, curriculum, curriculum_title, title, class_type} = template_vars;

		if(classes[0]['class_type'] == 'Summer Camp'){
			classes[0]['dance_category'] = `${age_min}-${age_max} Summer Camp`;
		}

		let date_range = '';
		if(classes[0]['start_day'] != null && classes[0]['class_type'] != 'Summer Camp'){
			date_range = "<h6>" + classes[0]['start_day'] + ' - ' + classes[0]['end_day'] + "</h6>";
		}
...
<div class="classes-item">
	...
	<div class="ci-text">
		<h4>${title}<br><div style="font-size:20px">(${class_type})</div></h4>
		<h6 style="margin-bottom:15px">${sale_price && sale_price !== 0 && sale_price !== "" ? `<span class="sale-price">$${sale_price.toFixed(2)}</span> <span class="original-price">$${price.toFixed(2)}</span>` : `$${price.toFixed(2)}`} ${payment_frequency}</h6>
		${date_range}
		${times}
	</div>
	<div class="ci-bottom">
		<div class="ci-author">
			<div class="author-text">
				...
				<h6><a target="_blank" href="${curriculum}">${curriculum_title}</a></h6>
			</div>
		</div>
...
function populate_free_classes(response){
	cls_options = [];
	$.each(response, function(_, cls){
		let id = cls['id'];
		let title = cls['title'];
		$('#freeclassoption').append($('<option>', {
			value: id,
			text: title
		}));
	});
...
function get_free_classes(){
	url = "/get_free_classes/"
	$.get(url, function(response){
		populate_free_classes(response);
	});
}
